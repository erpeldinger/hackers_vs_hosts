...
val find_bellman: (int*int*int) graph -> id -> id -> (id * id * int) list 
val gr_ecart_MFCM : (int * int * int) graph -> (int * int) graph

val flow_max_cout_min: (int * int * int) graph -> id -> id -> int*int

(* Return the flow variation of a path *)
val get_vflow: ('a * 'a * int) list -> int
val get_vflow_bf : (int * int * 'b) graph -> (int * int * 'c) list -> int
...
val update_graph_BF : (int * int * 'a) Graph.graph -> (int * int * 'b) list -> int -> (int * int * 'a) Graph.graph
